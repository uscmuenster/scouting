...
def _load_manual_stats_totals() -> Dict[
    ...
    manual_payload = manual_stats.load_manual_stats_directory()
    manual: Dict[
        str,
        List[Tuple[Tuple[str, ...], str, MatchStatsMetrics, Tuple[MatchPlayerStats, ...]]],
    ] = {}
    for stats_url, team_entries in manual_payload.items():
        teams_entries: List[
            Tuple[Tuple[str, ...], str, MatchStatsMetrics, Tuple[MatchPlayerStats, ...]]
        ] = []
        for team_entry in team_entries:
            ...
            if not name:
                continue
            serve = team_entry.serve or {}
            reception = team_entry.reception or {}
            attack = team_entry.attack or {}
            block = team_entry.block or {}
            try:
            ...
            for player_entry in team_entry.players:
                metrics_payload = player_entry.metrics or {}
                try:
                ...
                players.append(
                    MatchPlayerStats(
                        team_name=name,
                        player_name=pretty_name(player_entry.name),
                        jersey_number=player_entry.jersey_number,
                        metrics=player_metrics,
                        total_points=player_entry.total_points,
                        break_points=player_entry.break_points,
                        plus_minus=player_entry.plus_minus,
                    )
                )
            normalized_keys: List[str] = []
            primary_key = normalize_name(name)
            normalized_keys.append(primary_key)
            for alias in team_entry.aliases:
...
def build_html_report(
    *,
    ...
    _append_team_config(
        "wiesbaden",
        "VC Wiesbaden",
        "data/wiesbaden_stats_overview.json",
        wiesbaden_scouting,
    )
    _append_team_config(
        "erfurt",
        "Schwarz-Weiß Erfurt",
        "data/erfurt_stats_overview.json",
        erfurt_scouting,
    )
