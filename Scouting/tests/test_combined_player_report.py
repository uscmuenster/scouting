...
def _write_sample_csv(path: Path) -> None:
    path.write_text(
        """data_sources,match_number,match_id,kickoff,is_home,team,opponent,opponent_short,host,location,result_summary,player_name,jersey_number,total_points,break_points,plus_minus,serves_attempts,serves_errors,serves_points,receptions_attempts,receptions_errors,receptions_positive,receptions_perfect,receptions_positive_pct,receptions_perfect_pct,attacks_attempts,attacks_errors,attacks_blocked,attacks_points,attacks_success_pct,blocks_points,stats_url,csv_path
pdf;csv,42,98765,2024-03-18T19:30:00+01:00,True,USC Münster,VC Wiesbaden,Wiesbaden,USC Münster,Halle Berg Fidel,3:1,Max Mustermann,12,18,6,5,22,3,5,18,2,12,6,0.67,0.33,25,4,2,19,0.6,3,https://example.com/stats.pdf,docs/data/csv/sample.csv
""",
        encoding="utf-8",
    )
...
def test_render_combined_player_html(tmp_path: Path) -> None:
    ...
    assert "Ja" in html  # formatted boolean
